...
export default function Home() {
  ...
  // --- FITUR DARI FILE 2: STATE MODE REGISTRASI TERPISAH & SEARCH MODAL ---
  const [showRegisterModal, setShowRegisterModal] = useState(false);
  const [registerType, setRegisterType] = useState('siswa'); // 'siswa' atau 'guru'
  const [modalSearchQuery, setModalSearchQuery] = useState(''); // Fitur Pencarian di Modal Registrasi
  ...
  // FITUR DARI FILE 2: POPUP SWEETALERT REALTIME NOTIFIKASI WA TERKIRIM DENGAN NOMOR DETAIL
  ...
  // SAVE REGISTER CARD (TERHUBUNG KE PILIHAN SISWA / GURU)
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterJurusan !== 'Semua Jurusan') {
      if (filterJurusan === 'Guru / Staff') list = list.filter((s) => s.isGuru || s.kelas === 'Guru / Staff');
        ...
        list = list.filter((s) => (s.kelas || '').toLowerCase().includes(filterJurusan.toLowerCase()));
      }
    }
  ...
  return (
    <div style={styles.dashboardContainer}>
      ...
      {/* FITUR DARI FILE 2: MODAL REGISTRASI KARTU RFID DENGAN TAB SWITCHING & CARI */}
      {showRegisterModal && (
