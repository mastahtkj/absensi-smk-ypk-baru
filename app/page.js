...
// Fungsi pembantu untuk memformat nomor HP ke format internasional (62xxxx)
function formatPhoneNumber(no) {
  if (!no) return null;
...
async function sendWaNotification(noHp, nama, status, waktu) {
  ...
  if (!token || !formattedPhone) return;
  ...
  try {
    ...
    console.error('Gagal kirim Notifikasi WA via Fonnte:', err);
  }
}
...
export async function POST(req) {
  try {
    ...
    // 5. Kirim Notifikasi WA secara Async jika nomor terdaftar
    if (noHp) {
      sendWaNotification(noHp, nama, status, wibTimeString).catch(err => console.error(err));
    }
